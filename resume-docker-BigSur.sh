...
echo "system will hook PID 1 (Crtl-Alt-F3) and or (Crtl-Alt-F4) then back to (Crtl-Alt-F1) to release XDM login for fedora-mac script will pause for 5 seconds"
sleep 5s
...
'
echo 'ssh -p 2022 -Y -X root@172.17.0.1'
sleep 10s
ssh -p 2022 -Y -X root@172.17.0.1
